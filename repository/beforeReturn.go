...
func (repo repositoryDB) CreateSaleReturn(ctx context.Context, order request.BeforeReturnOrder) (*response.BeforeReturnOrderResponse, error) {
    ...
    `
	if _, err = tx.NamedExecContext(ctx, queryOrder, order); err != nil {
...
func (repo repositoryDB) UpdateSaleReturn(ctx context.Context, req request.UpdateSaleReturn) (*response.UpdateSaleReturnResponse, error) {
	tx, err := repo.db.BeginTxx(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to start transaction: %w", err)
	}

	// ใช้ defer เพื่อให้แน่ใจว่ามีการ rollback ถ้าหากเกิด panic หรือ error
	defer func() {
		if p := recover(); p != nil {
			_ = tx.Rollback()
			panic(p)
	...
	// Execute SQL โดยใช้ NamedExecContext ของ sqlx
...
func (repo repositoryDB) CancelSaleReturn(ctx context.Context, orderNo string, updateBy string, remark string) error {
	...
	err = tx.GetContext(ctx, &statusConfID, checkQuery, map[string]interface{}{"OrderNo": orderNo})
	if err == sql.ErrNoRows {
		return fmt.Errorf("order not found: %s", orderNo)
	}
	if err != nil {
	...
	`
	var cancelID int
	err = tx.GetContext(ctx, &cancelID, insertCancelStatus, map[string]interface{}{
		"OrderNo":  orderNo,
		"Remark":   remark,
		"CancelBy": updateBy,
	})
	...
	updateOrder := `
		...
		WHERE OrderNo = :OrderNo
	`
	result, err := tx.ExecContext(ctx, updateOrder, map[string]interface{}{
		"OrderNo":  orderNo,
		"CancelID": cancelID,
		"UpdateBy": updateBy,
	})
