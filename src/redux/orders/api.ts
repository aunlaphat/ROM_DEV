...
// Search Order Saga
export function* searchOrderSaga(action: {
  type: typeof OrderActionTypes.SEARCH_ORDER_REQUEST;
  ...
  try {
    ...
    const response = yield call(() =>
      GET(`/order/search?${queryParams.toString()}`)
    );
...
export function* createReturnOrderSaga(action: {
  type: typeof OrderActionTypes.CREATE_RETURN_ORDER_REQUEST;
  payload: CreateReturnOrderRequest;
}): SagaIterator {
  try {
    openLoading();
...
export function* generateSrSaga(action: {
  type: typeof OrderActionTypes.GENERATE_SR_REQUEST;
  ...
  try {
    ...
    const srNo = response.data.data;
    logger.api.success(`/order/generate-sr/${orderNo}`, srNo);
...
export function* updateSrSaga(action: {
  type: typeof OrderActionTypes.UPDATE_SR_REQUEST;
  payload: UpdateSrRequest;
}): SagaIterator {
  try {
    openLoading();
...
export function* updateStatusSaga(action: {
  type: typeof OrderActionTypes.UPDATE_STATUS_REQUEST;
  ...
  try {
    ...
    const response = yield call(() =>
      POST(`/order/update-status/${orderNo}`, {
        orderNo,
        roleID,
        userID,
      })
...
export function* cancelOrderSaga(action: {
  type: typeof OrderActionTypes.CANCEL_ORDER_REQUEST;
...
export function* markOrderEditedSaga(action: {
  type: typeof OrderActionTypes.MARK_EDITED_REQUEST;
  payload: string;
}): SagaIterator {
  try {
    openLoading();
    ...
    const response = yield call(() =>
      PATCH(`/order/mark-edited/${orderNo}`, {})
    );
